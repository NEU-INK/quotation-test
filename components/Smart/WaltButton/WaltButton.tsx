...
const WaltButton: React.FC<ButtonProps> = ({
    ...
    const renderIcon = () => {
        ...
        if (isActive && iconActive) {
            return typeof iconActive === "string" ? <img src={iconActive} alt="active-icon" className={styles.icon} /> : iconActive;
        }
        if (isHovered && iconHover) {
            return typeof iconHover === "string" ? <img src={iconHover} alt="hover-icon" className={styles.icon} /> : iconHover;
        }
    ...
    const ButtonContent = (
        <button
            ...
            style={{
                padding: "10px 20px",
                borderRadius: "5px",
                border: `1px solid ${isActive ? activeBorderColor : isHovered ? hoverBorderColor : borderColor}`,
        ...
        >
            {icon && iconPosition === "left" && <span className={styles.iconWrapper}>{renderIcon()}</span>}
            <span>{text}</span>
            {icon && iconPosition === "right" && <span className={styles.iconWrapper}>{renderIcon()}</span>}
        </button>
    );
