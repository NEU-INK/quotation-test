...
const ProductsList: React.FC = () => {
    ...
    const BreadcrumbsNav = () => (
        <Breadcrumbs separator=">" aria-label="breadcrumb" className={styles.Breadcrumbs}>
            <Link onClick={() => handleCategoryClick('All')} underline="hover" color={selectedCategory === 'All' ? 'text.primary' : 'inherit'}>
                All
            </Link>
                ...
                <Box  key={key}
                    ...
                    <Box key={product.id} className={styles.ProductCard}>
                        ...
                        <Box className={styles.ProductInfo}>
                            <Typography className={styles.ProductTitle} fontSize={'2.9rem'}  fontWeight={"bold"}>{product.title}</Typography>

                            <Typography fontSize={'2.2rem'} noWrap fontWeight={"bold"}>{product.price}</Typography>
    ...
    return (
        <Box className={styles.Box}>
            <Box className={styles.BoxTitle}>
                <Typography lineHeight={1} fontSize={`${isMobile?'4rem':'9.6rem'}`} fontWeight={800} fontFamily={'HC'}>
                    PRODUCTS
                </Typography>
            </Box>
                    ...
                    <Box key={category.category} >
                        ...
                        <Box
                            ...
                            sx={{fontSize:'2.76rem',
                                ...
                                lineHeight:2,
                                cursor:'pointer',
                                borderRadius:`${index === 0 ?  '10px 0 0 10px' : `${index === categories.length-1 ?  '0 10px 10px 0' : ''}`}`,
                                fontWeight:800,padding:'0.4rem 4rem',background:`${selectedCategory === category.category ?  '#003680' : '#ffffff'}`,color:`${selectedCategory === category.category ?  '#fff' : '#13357b'}`}}
                        >
                            {category.category}
                        </Box>
                        {selectedCategory === category.category && showSubcategories && category.subcategories.length > 0 && (
                            <Box className={styles.SubcategoryMenu}>
                                {category.subcategories.map(subcategory => (
                                    ...
                                    <MenuItem
                                        ...
                                        className={`${selectedSubcategory === subcategory ? styles.SelectedSubcategory : ''} ${styles.btnZ}`}


                                        sx={{fontSize:'2.6rem', fontWeight:400,color:'#13357b',borderRadius:'5%',padding:'0.4rem 4rem'}}
                                    >
                                        {subcategory}
                                    </MenuItem>
                                ))}
            ...
            {isSX && (
                <Box className={styles.SortOptions}>
                    <IconButton onClick={() => setSortOption({ field: 'price', order: sortOption.order === 'asc' ? 'desc' : 'asc' })}>
                        {sortOption.order === 'asc' ? <ArrowUpward /> : <ArrowDownward />}
                        按价格排序
                    </IconButton>
                    <IconButton onClick={() => setSortOption({ field: 'name', order: sortOption.order === 'asc' ? 'desc' : 'asc' })}>
                        {sortOption.order === 'asc' ? <ArrowUpward /> : <ArrowDownward />}
                        按名称排序
                    </IconButton>
                </Box>
