...
const AboutUsOurStory: React.FC = () => {
    ...
    useEffect(() => {
        if (carouselRef.current) {
            ...
            const centerPosition = itemHeight * selectedIndex - (carouselRef.current.clientHeight / 2) + (itemHeight / 2);
    ...
    return (
        ...
        >
            <Box className={styles.top}>
                ...
                <Typography fontFamily={'HS'} fontSize={`${isMobile?'2rem':'2.6rem'}`} style={{ textAlign: 'right' }} lineHeight={1}>
            ...
            <Box className={styles.cardsContainer}>
                <Box className={styles.leftCard}>
                    <Box className={styles.imageFrame}>
                        ...
                        <Image src={data[selectedIndex].img} alt={data[selectedIndex].title} width={1000}
                               height={1000} style={{ width: 'auto', height: "auto",maxWidth:'100%',maxHeight:'100%' }} />
                    </Box>
                    <Box className={styles.textContent}>
                        <Box className={styles.titleBox}>
                            <Typography className={styles.title}   fontSize={`${isMobile?'2.2rem':'3.6rem'}`}  fontWeight={800}>
                                {data[selectedIndex].title}
                                <sup  style={{ fontSize: `${isMobile?'1.6rem':'2rem'}` }}>[{data[selectedIndex].sup}]</sup>
                            </Typography>
                        </Box>
                        ...
                        <Box className={styles.describeBox}>
                            ...
                            <Box className={`${isMobile? styles.describeBoxBoxMobile:styles.describeBoxBox}`}>
                                {data[selectedIndex].describe.map((desc) => (
                                    <Typography
                                        fontFamily={'HS'}
                                        fontSize={`${isMobile?'2rem':'2.6rem'}`} lineHeight={1.25} key={desc.num}  className={styles.describeText} sx={{ marginTop: '4rem' }}>
                                        {desc.text}
                                    </Typography>
                                ))}
                            </Box>
                ...
                <Box className={styles.carousel} ref={carouselRef}>
                    <Box className={styles.imageList}>
                        {data.map((item, index) => (
                            <Box
                                key={item.id}
                                className={`${styles.carouselItem} ${selectedIndex === index ? styles.selected : ''}`}
