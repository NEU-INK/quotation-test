    ...
    return (
        <>
            ...
            <Box className={styles.newsBox}>
                <Typography lineHeight={2} fontSize={`${isMobile?'4rem':'9.6rem'}`} fontWeight={800} fontFamily={'HC'}>
                    NEWS AND EVENTS
                </Typography>
                            ...
                            <Box className={styles.imageContainer}>
                                ...
                                <Box className={styles.overlay}>
                                    <Button
                                        ...
                                        sx={{ fontSize: `${isMobile?'2rem':'2.6rem'}`, textTransform: 'none', fontFamily: 'HS', padding: '0.2rem 3rem' }}
                                    >
                                        Read More →
                                    </Button>
                                </Box>
                            </Box>
                            <Typography className={styles.newsTitle} fontSize={`${isMobile?'3rem':'3.6rem'}`} fontWeight={800} fontFamily={'HS'}>
                                {post.frontmatter.title}
                            </Typography>
                            <Typography fontSize={`${isMobile?'2rem':'2.6rem'}`} className={styles.newsExcerpt} fontFamily={'HS'}>
                                {post.frontmatter.excerpt}
                            </Typography>
                            <Box className={styles.newsFooter}>
                                <Box className={styles.authorContainer}>
                                    <Box className={styles.authorCircle} fontSize={`${isMobile?'1.8rem':'1.8rem'}`}>
                                        {post.frontmatter.author.split(' ').map(word => word[0].toUpperCase()).join('').substring(0, 2)}
                                    </Box>
                                    <Typography fontSize={`${isMobile?'2rem':'2.4rem'}`} className={styles.authorName}>
                                        {post.frontmatter.author}
                                    </Typography>
                                </Box>
                                <Typography fontSize={`${isMobile?'2rem':'3.6rem'}`}  className={styles.postDate}>
