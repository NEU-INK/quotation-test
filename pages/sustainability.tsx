        ...
        secondaryTitles: [
            {
                id: 1,
                title: 'HUMAN',
                description: 'Human development through education, training and equipping the local community.',
            ...
            },
            {
                id: 3,
                title: 'ANIMALS',
                description: 'We are committed to animal welfare and support sustainable practices that minimize harm to animals.',
                imageSrc: '/images/sustainability/carbon-zero-eco-park-2.png'
            },
            {
                id: 4,
                title: 'NATURE',
                description: 'We prioritize biodiversity conservation and support initiatives that protect natural habitats.',
...
const Sustainability = () => {
  ...
  // };
    ...
    const handleSecondaryClick = (secondary: { id: number; title: string; description: string; imageSrc: string }) => {
        ...
        if (scrollContainerRef.current && scrollContainerRefBox.current) { // 添加 null 检查
    ...
    return (
        <>
            <Head>
                ...
                {/* Open Graph Metadata */}
                <meta property="og:title" content="Sustainability | Walt Tec | Eco-friendly Manufacturing" />
                <meta
                ...
                <meta name="twitter:title" content="Sustainability | Walt Tec | Eco-friendly Manufacturing" />
                <meta
            ...
            <Box className={`${styles.sustainabilityPage}`}>
                ...
                <Box className={styles.park} display="flex">
                    {/* Left Side (50%) */}
                    <Box flex="1" padding=" 0 2%" display="flex" flexDirection="column" justifyContent="space-between">
                        ...
                        <Box display="flex">
                            {/* Primary Menu (30%) */}
                            <Box width="30%" paddingRight="10px"
                            ...
                            <Box style={{height: '37.8rem', overflow: 'hidden'}}
                                 width="60%" paddingLeft="10px" ref={scrollContainerRefBox}>
                                <Box style={{height: '54rem', padding: '16.2rem 0', overflow: 'auto'}}
                        ...
                        <Box style={{overflowY: 'auto', maxHeight: '150px', marginTop: '20px'}}>
                            <Typography fontSize={`${isMobile ? '2rem' : '2.6rem'}`}
                ...
                <Box className={styles.materials}>
                    <Typography fontSize={`${isMobile ? '4rem' : '9.6rem'}`} lineHeight={2} fontWeight={800}
                                fontFamily={'HC'} textAlign={'center'}>
                        ...
                        {data.map((item, index) => (
                            <Box key={index} className={styles.materialsCard}>
                                <Paper elevation={0} className={styles.paper}>
                                    ...
                                    <Box className={styles.materialsText}>
                                        <Box className={styles.textContainer}> {/* 外层负责居中且换行左对齐 */}
                                            <Typography
   ...
   <span>     [{item.id}]</span>
                ...
                <Box className={styles.invitation}>
                    ...
                    <Typography fontSize={`${isMobile ? '3rem' : '4.9rem'}`} lineHeight={1.2} fontWeight={800}
                                fontFamily={'HS'}>
                        Achieving a carbon neutral world is one of the <br/> most important objectives of our time.
                    </Typography>
                    <Typography mt={'2.3rem'} fontSize={`${isMobile ? '2rem' : '3.6rem'}`} lineHeight={1.2}
                                fontFamily={'HS'}>
                        We are always looking for sustainability partners to join us on this<br/> journey including
                        carbon accounting, certification, engineering, <br/>renewable energy, new eco-friendly materials
                        and
                        any contribution to<br/> sustainable development goals.
                    </Typography>
...
<Box style={{display:'flex',justifyContent:'center',alignItems:'center',marginTop:'5rem'}}>
    <WaltButton
        ...
        style={{    fontSize: `${isMobile ? '2rem' : '2.6rem'}`,
            fontWeight: 600,width:'340px' }} // 传入自定义样式
    />
</Box>
